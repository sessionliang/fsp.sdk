 ...
 * @return string
 */
function generate_sign(array $attributes, string $secretKey, $encryptMethod = 'md5')
{
    ksort($attributes);
    $query_string = urldecode(http_build_query($attributes));
    if(get_magic_quotes_gpc()){$query_string = stripslashes($query_string);}
    $temp_string = $query_string.$secretKey;
    return call_user_func_array($encryptMethod, [$temp_string]);
}
